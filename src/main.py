...
def guess_filename_from_url(url: str) -> str:
    # naive but works for most direct links
...
def main() -> int:
    ...
    parser.add_argument("--limit", type=int, default=None,
                        help="Limit number of downloads")   # ✅ ADD THIS LINE
    ...
    for idx, item in enumerate(found, start=1):
        ...
        # Create dataset folder name (stable)
        # If no title/dataset_url, create a slug from filename
        ...
        filename = getattr(item, "filename", None) or guess_filename_from_url(item.qda_url)
        ...
        if res.ok and ok_insert:
            ...
            skipped += 1
            logger.info(f"[{idx}/{len(found)}] SKIPPED (duplicate)")
